...
def local_pca(pts, idx):
    nb = gather_nb(pts, idx)
    cen = nb.mean(2)
    dif = nb - cen.unsqueeze(2)
    cov = dif.transpose(-1, -2) @ dif / idx.shape[-1]
    ev, evec = torch.linalg.eigh(cov)
    return nb, cen, dif, ev.clamp_min(1e-12), evec
...
def geo_features(pts):
    ...
    for k in SCALES:
        idx = knn_idx(pts, k)
        nb, cen, dif, ev, evec = local_pca(pts, idx)
        ...
        feats += [ev / s, torch.log(s), off, noff, dots.mean(-1, keepdim=True), dots.min(-1, keepdim=True).values,
                  torch.log(rad)]
    base_nrm = local_pca(pts, knn_idx(pts, NORMAL_K))[4][..., 0]
    kmax_list = []
    for k in CURV_SCALES:
        idx = knn_idx(pts, k)
        nb, cen, dif, ev, evec = local_pca(pts, idx)
        rad = dif.norm(dim=-1).mean(-1).clamp_min(1e-9).unsqueeze(-1)
        rel = nb - pts.unsqueeze(2)
        u = (rel * evec[..., 2].unsqueeze(2)).sum(-1) / rad
        v = (rel * evec[..., 1].unsqueeze(2)).sum(-1) / rad
        w = (rel * evec[..., 0].unsqueeze(2)).sum(-1) / rad
        A = torch.stack([u * u, u * v, v * v, u, v, torch.ones_like(u)], -1)
        AtA = A.transpose(-1, -2) @ A + 1e-4 * torch.eye(6, device=pts.device)
        sol = torch.linalg.solve(AtA, (A.transpose(-1, -2) @ w.unsqueeze(-1)))[..., 0]
        res = ((A @ sol.unsqueeze(-1))[..., 0] - w).pow(2).mean(-1, keepdim=True).sqrt()
        a, b, c = sol[..., 0], sol[..., 1], sol[..., 2]
        mean_c = a + c
        disc = ((a - c).pow(2) + b.pow(2)).sqrt()
        k1, k2 = (mean_c + disc).abs(), (mean_c - disc).abs()
        kmax, kmin = torch.maximum(k1, k2), torch.minimum(k1, k2)
        kmax_list.append(kmax)
        nbn = gather_nb(base_nrm, idx)
        gm = torch.linalg.eigvalsh(nbn.transpose(-1, -2) @ nbn / idx.shape[-1])
        feats += [torch.log1p(kmax).unsqueeze(-1), torch.log1p(kmin).unsqueeze(-1),
                  (kmin / (kmax + 1e-3)).unsqueeze(-1), torch.log1p(res * 100), gm[..., :2]]
    idx = knn_idx(pts, CURV_SCALES[1])
    kn = gather_nb(kmax_list[1].unsqueeze(-1), idx)[..., 0]
    feats += [(kn.std(-1) / (kn.mean(-1) + 1e-3)).unsqueeze(-1)]
...
def main():
    ...
    print("feature dims", allf.shape[1], "nonfinite", int((~torch.isfinite(allf)).sum()), flush=True)
